...
def padronizar_clientes(caminho: str) -> pd.DataFrame:
    ...
    if "Email_Cliente"         in df.columns: df["Email_Cliente"]         = limpar_email(df["Email_Cliente"])
    if "Data_Cadastro_Cliente" in df.columns: df["Data_Cadastro_Cliente"] = limpar_data(df["Data_Cadastro_Cliente"])
    antes = len(df)
    ...
    cols = [c for c in ["Nome_Cliente", "Email_Cliente", "Cidade_Cliente", "Data_Cadastro_Cliente"] if c in df.columns]
...
def padronizar_produtos(caminho: str) -> pd.DataFrame:
    ...
    if "Preco_Produto" in df.columns: df["Preco_Produto"] = limpar_decimal(df["Preco_Produto"])
...
def padronizar_pedidos(caminho: str) -> pd.DataFrame:
    ...
    if "Data_Pedido"        in df.columns: df["Data_Pedido"]        = limpar_data(df["Data_Pedido"])
    if "Valor_Total_Pedido" in df.columns: df["Valor_Total_Pedido"] = limpar_decimal(df["Valor_Total_Pedido"])
...
def padronizar_itens_pedido(caminho: str) -> pd.DataFrame:
    ...
    if "Quantidade_Item"     in df.columns: df["Quantidade_Item"]     = limpar_inteiro(df["Quantidade_Item"])
    if "Preco_Unitario_Item" in df.columns: df["Preco_Unitario_Item"] = limpar_decimal(df["Preco_Unitario_Item"])
...
def importar_csv(caminho: str, tabela: str = None) -> int:
    ...
    df = funcao(caminho)
    return inserir_no_banco(df, tabela)
...
def classificar_risco_pedidos() -> dict:
    ...
    """)
    if len(df) < 10:
        return {"erro": "Dados insuficientes (mínimo 10 pedidos)."}
